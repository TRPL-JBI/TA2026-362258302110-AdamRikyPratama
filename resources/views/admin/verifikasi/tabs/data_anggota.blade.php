...
<div class="card border-0">
    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center rounded-top">
        ...
        <span class="badge bg-light text-dark fs-6 rounded-pill">
            {{ $anggota_terurut->count() }} Orang
        </span>
    </div>

    <div class="card-body p-4">
        {{-- ALERT jika belum ada anggota --}}
        @if ($anggota_terurut->count() == 0)
            <div class="alert alert-warning shadow-sm">
                <h6><i class="fas fa-exclamation-triangle me-2"></i>Informasi</h6>
                <p>Organisasi <strong>{{ $organisasi->nama }}</strong> belum memiliki anggota.</p>
            </div>
        @endif
        ...
        <div class="table-responsive mb-5">
            <table class="table table-bordered table-hover align-middle">
                ...
                <tbody>
                    @forelse ($anggota_terurut as $index => $anggota)
                        <tr>
                            ...
                            <td class="text-center">
                                @if ($anggota->umur)
                                    <span class="badge bg-secondary">{{ $anggota->umur }} th</span>
                                @else
                                    <span class="text-muted">-</span>
                                @endif
                            </td>
                            ...
                            <td>
                                @if ($anggota->whatsapp || $anggota->telepon)
                                    <div>
                                        @if ($anggota->whatsapp)
                                            <small><i
                                                    class="fab fa-whatsapp text-success me-1"></i>{{ $anggota->whatsapp }}</small><br>
                                        @endif
                                        @if ($anggota->telepon && $anggota->telepon != $anggota->whatsapp)
                                            <small><i
                                                    class="fas fa-phone text-muted me-1"></i>{{ $anggota->telepon }}</small>
                                        @endif
                                    </div>
                                @else
        ...
        <form action="{{ route('admin.verifikasi.store', $organisasi->id) }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" {{ $ketua ? 'checked' : '' }} disabled>
                        <label class="form-check-label">Struktur Ketua {{ $ketua ? '✓' : '✗' }}</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" {{ $sekretaris ? 'checked' : '' }} disabled>
                        <label class="form-check-label">Struktur Sekretaris {{ $sekretaris ? '✓' : '✗' }}</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" {{ $bendahara ? 'checked' : '' }} disabled>
                        <label class="form-check-label">Struktur Bendahara {{ $bendahara ? '✓' : '✗' }}</label>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-3 mt-3">
                ...
                <textarea name="catatan" class="form-control" rows="2">{{ $verifikasiData->where('tipe', 'data_anggota')->first()->catatan ?? '' }}</textarea>
            </div>

            <div class="mb-3">
                <label class="form-label fw-bold"><i class="fas fa-comment-dots me-1"></i>Keterangan untuk
                    Pendaftar</label>
                <textarea name="keterangan" class="form-control" rows="3">{{ $verifikasiData->where('tipe', 'data_anggota')->first()->keterangan ?? '' }}</textarea>
